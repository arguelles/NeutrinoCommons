...
gsl_matrix_complex* flavorM2(PhysConst param){
    ...
    gsl_matrix_complex *T3 = gsl_matrix_complex_alloc(numneu,numneu);
...
static int RHS_GSL(double x,const double *neuvec_real_in, double *neuvec_real_out,void *params_void){
    // typing params
...
void CalNeuOscGSLML(int ineu,double E,string body_name,double body_params[],long body_params_len,double track_params[],long track_params_len,string neutype,double abs_error,double rel_error,bool return_state,bool optimization){
    ...
    if (body_name == "Vacuum"){
        ...
        if (body_params_len == 0){
            ...
            // body_name + "received to many arguments";
        }
        
        if (track_params_len == 2){
            Track track(track_params[0],track_params[1]);
        } else {
            // "track received wrong number of arguments for body_name";
        }
    } else {
        // "body not available"
        break;
